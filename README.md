...
- Hash
  ...
  - mbo/hash:hash_cc, mbo/hash/hash.h
    ...
    - struct `Hasher<Algo>`: Completes any `IsHashAlgorithm` struct into the full `GetHash64`/`GetHash128`/`GetHash` interface, synthesizing what is missing (fold for 64, two seeded passes for 128, mangle for `GetHash`).
