...
TEST(ParseChecksumLineTest, AcceptsTextAndBinaryMarkers) {
  EXPECT_THAT(ParseChecksumLine("01ab  file.txt", 4), Optional(ChecksumLine{"01ab", "file.txt"}));
  EXPECT_THAT(ParseChecksumLine("ABCD *file.txt", 4), Optional(ChecksumLine{"ABCD", "file.txt"}));
}
