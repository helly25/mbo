...
std::pair<std::size_t, std::size_t> ExpandWhiteSpace(std::string_view output,
                                                     std::size_t tag_pos,
                                                     std::size_t tag_len) {
...
std::pair<std::size_t, std::size_t> FindTag(std::string_view output,
                                            std::string_view tag) {
...
void AbslStringify(Sink &sink, const Template::TagType &value) {
  absl::Format(&sink, "%s", [value] {
    switch (value) {
    case Template::TagType::kValue:
      return "TagType::Value";
    case Template::TagType::kSection:
      return "TagType::Section";
    case Template::TagType::kControl:
...
Template *Template::AddSectionDictionary(std::string_view name) {
  ...
  auto [it, inserted] = data_.emplace(
      name, TagData<SectionDictionary>{.tag = std::move(tag), .data = {}});
  auto &dict = std::get<TagData<SectionDictionary>>(it->second).data;
  return &dict.emplace_back();
}

absl::Status Template::SetValue(std::string_view name, std::string_view value,
                                bool allow_update) {
  TagInfo tag{
      .name{name},
      .start = absl::StrCat("{{", name, "}}"),
  ...
  auto [it, inserted] = data_.emplace(
      name, TagData<std::string>{.tag = std::move(tag), .data{value}});
  if (inserted) {
  ...
  if (allow_update) {
    ...
    if (data_str == nullptr) {
      return absl::AlreadyExistsError(absl::StrCat(
          "A value for '", name, "' already exists with a different type."));
    }
    data_str->data.assign(value);
    return absl::OkStatus();
  }
  return absl::AlreadyExistsError(
      absl::StrCat("A value for '", name, "' already exists."));
}

std::optional<const Template::TagInfo>
Template::FindAndConsumeTag(std::string_view *pos, bool configured_only) {
  ...
  static constexpr LazyRE2 kConfiguredTags = {
      "({{(#?)(\\w+)(?:=((?:[^}]|[}][^}])+))?}})"};
  ...
  return TagInfo{
      ...
      .config{config},
      .type = type.empty()
                  ? (config.empty() ? TagType::kValue : TagType::kControl)
                  : TagType::kSection,
  };
}

absl::Status Template::ExpandTags(
    bool configured_only, std::string &output,
  ...
  while (true) {
    ...
    if (tag.type == TagType::kSection && data_.find(tag.name) != data_.end()) {
      return absl::InvalidArgumentError(
          absl::StrCat("Tag name '", tag.name, "' appears twice."));
    }
    const std::size_t tag_pos =
        pos.data() - output.c_str() - tag.start.length();
    const auto [replace_pos, replace_tag_len] =
    ...
    case TagType::kSection: {
      const auto tag_end_pos =
          output.find(tag.end, replace_pos + replace_tag_len);
      if (tag_end_pos == std::string_view::npos) {
        return absl::InvalidArgumentError(absl::StrCat(
            "Tag name '", tag.name, "' has no end tag '", tag.end, "'."));
      }
      const auto [replace_end, replace_end_len] =
          ExpandWhiteSpace(output, tag_end_pos, tag.end.length());
      replace_len = replace_end + replace_end_len -
                    replace_pos; // whole replace incl. tags
      replace_str =
          output.substr(replace_pos + replace_tag_len,
                        replace_len - replace_tag_len - replace_end_len);
      MBO_STATUS_RETURN_IF_ERROR(func(tag, replace_str));
      break;
    }
    case TagType::kValue: {
      ...
      if (it != data_.end() &&
          std::get_if<TagData<std::string>>(&it->second) != nullptr) {
        replace_len = replace_tag_len;
...
absl::Status Template::MaybeLookup(const TagInfo &tag_info,
  ...
  if (data_it == data_.end()) {
    return absl::NotFoundError(absl::StrCat("Tag '", tag_info.name,
                                            "' references '", data,
                                            "' which was not found."));
  }
  ...
  return absl::InvalidArgumentError(
      absl::StrCat("Tag '", tag_info.name, "' refrences '", data,
                   "' which has unsupported data type."));
}

absl::Status Template::MaybeLookup(const TagInfo &tag_info,
                                   std::string_view data, int &value) const {
  ...
  if (const auto *tag_data = std::get_if<TagData<std::string>>(&found)) {
    ...
    return absl::InvalidArgumentError(
        absl::StrCat("Tag '", tag_info.name, "' refrences '", data,
                     "' which has non numeric value '", tag_data->data, "'"));
...
absl::Status Template::ExpandRangeTag(const TagInfo & /*tag*/, Range &range,
                                      std::string &output) {
  const std::string original = std::move(output);
  output = "";
  if (range.step == 0) {
    return absl::InternalError(
        absl::StrCat("A range should never have step == 0."));
  }
  range.expanding = true;
  const absl::Cleanup cleanup = [&range] { range.expanding = false; };
  for (range.curr = range.start;
       range.step > 0 ? range.curr <= range.end : range.curr >= range.end;
...
absl::Status Template::ExpandRangeData(const TagInfo &tag,
  ...
  if (range.step == 0) {
    return absl::InvalidArgumentError(
        absl::StrCat("Tag '", tag.name, "' cannot have step == 0."));
  }
  // We could check:
  // * range.step > 0 && range.start > range.end
  // * range.step < 0 && range.start < range.end
  // But that would not work well with values that were looked up or computed.
  auto [r_tag_it, inserted] =
      data_.emplace(tag.name, TagData<Range>{.tag = tag, .data = range});
  if (!inserted) {
    return absl::InvalidArgumentError(
        absl::StrCat("Tag '", tag.name, "' appears twice."));
  }
...
Template::ParseStringList(std::string_view data) {
  ...
  for (std::size_t pos = 0; pos < data.size(); ++pos) {
    ...
    switch (chr) {
    default:
      break; // normal character, just add.
    case ',':
      if (!quote) {
        // A Split!
    ...
    case '\\': {
      ...
      case 'N': // \N{Name}: Named unicode char
        return absl::UnimplementedError(
            "StringList has not yet supported escape sequence.");
      default:
        return absl::InvalidArgumentError(
            "StringList has unsupported escape sequence.");
      } // switch for escape
    }   // case escape
    }   // switch char
    curr += chr;
...
absl::Status
Template::ExpandConfiguredSection(std::string_view name,
...
absl::Status Template::ExpandConfiguredList(const TagInfo &tag,
...
absl::Status Template::ExpandConfiguredTag(const TagInfo &tag,
                                           std::string &output) {
  ...
  return absl::UnimplementedError(absl::StrCat(
      "Tag '", tag.name, "' has unknown config format '", tag.config, "'."));
}
...
absl::Status Template::Expand(std::string &output) {
  ...
  for (auto &[name, data] : data_) {
    MBO_STATUS_RETURN_IF_ERROR(std::visit(
        [&](auto &typed_data) { return Expand(typed_data, output); }, data));
  }
  return RemoveTags(output);
}

absl::Status Template::Expand(TagData<SectionDictionary> &tag,
                              std::string &output) {
  if (absl::StrContains(output, absl::StrCat("{{", tag.tag.name, "}}"))) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Simple tag '", tag.tag.name, "' used in SectionDictionary."));
  }
  while (true) {
    ...
    if (pos_start == std::string::npos) {
      if (pos_end != std::string::npos) {
        return absl::InvalidArgumentError(absl::StrCat(
            "Section '", tag.tag.name, "' has end but no start marker."));
      }
      break;
    }
    if (pos_end == std::string::npos) {
      return absl::InvalidArgumentError(absl::StrCat(
          "Section '", tag.tag.name, "' has start but no end marker."));
    }
    if (pos_end < pos_start) {
      return absl::InvalidArgumentError(absl::StrCat(
          "Section '", tag.tag.name, "' has end before start marker."));
    }
    ...
    for (auto &section : tag.data) {
      std::string value = output.substr(
          pos_start + start_len, tmpl_len); // We still hold the "input" part.
...
absl::Status Template::Expand(const TagData<Range> &tag, std::string &output) {
  // Unlike when expanding sections, here the tag is both the section and the
  // value.
  if (!tag.data.expanding) {
    return absl::InvalidArgumentError(
        absl::StrCat("Tag '", tag.tag.name, "' cannot be expanded."));
  }
...
absl::Status Template::Expand(const TagData<std::string> &tag,
