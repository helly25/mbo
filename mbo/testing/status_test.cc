...
TEST_F(StatusMatcherTest, MessagesOfStatusPayloads) {
  ...
  }
  {
    ...
    EXPECT_THAT(
        ...
        Pair(
            false,
            "which has a non-matching payload map whose element #1 ((\"def\", \"texting\")) has a first field that isn't equal to \"url\", or has a second field that isn't equal to \"content\", whose first field does not match"));
    EXPECT_THAT(
