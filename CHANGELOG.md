...
- Sped up the `myers` tokenizer: interning keys are now `std::string_view`s into the preprocessed line cache hashed with `mbo::hash::GetHash64` (previously every line was copied into a `std::string`-keyed map), and `ignore_case` folds each line once into a reused buffer with stable storage only for distinct lines. Tokenize-heavy inputs (20k distinct 120-char lines) run ~16% faster; see the `tokenize_*` cases in `//mbo/diff:diff_benchmark`.
