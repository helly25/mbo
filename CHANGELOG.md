...
- Added `mbo::hash::GetHash64(std::string_view)` / `GetHash128(std::string_view)` and the `mbo::hash::Hash128` result type - a new constexpr-safe, non-cryptographic hash (namespace `mbo::hash::mh`), the initial default behind `mbo::hash::GetHash*` (later superseded by `rapidhash`, see below).
...
- Made `mh` (then the default) substantially faster (changing its values, as allowed by the stability contract): runtime loads now use `memcpy` (gcc never folded the byte-assembly loads - roughly 3x on gcc for **all** algorithms), tail loads use branch-lite overlapping reads, and `mh::GetHash128` processes large inputs (>= 64 bytes) in 32-byte stripes over 4 accumulators with a cross-mixed finalizer - about 2.5-3.7x throughput at >= 256 bytes, now beating `xxh64`.
...
- Changed the default algorithms: `GetHash64`/`GetHash`/`GetHash32` now default to `rapidhash` (SMHasher3-clean, canonical, best mixed-length latency) and `GetHash128` to the 128-bit-native `xxh3` (also SMHasher3-clean). The in-house `mh` stays available; an SMHasher3 evaluation (now in mbo/hash/README.md) found it not clean.
...
- Added `mbo::hash::mh2` (EXPERIMENTAL): an in-house redesign of `mh` around the widening 64x64->128 multiply ("MUM") - unrolled per-length small-key loads, a 128-byte 8-chain bulk fetch window, and a native dual-lane 128-bit form. **SMHasher3: PASS 188/188 in both widths** (the only clean 128-bit result measured on our rig; reached in three measured iterations documented in mbo/hash/README.md), fastest mixed-length latency at <= 16 bytes and fastest bulk throughput in the library. `mh`-vs-`mh2` retirement, default-algorithm, naming, and streaming decisions are tracked in mbo/hash/TODO.md.
