...
- Removed per-line copies from the `mbo::diff` preprocessing: `LineCache::processed` is now a `std::string_view` aliasing the input line (default path), a sub-range of it (`ignore_trailing_space` suffix trim, `strip_comments` truncation), or a rebuilt string in stable arena storage (`ignore_all_space`, `ignore_consecutive_space`, parsed comment stripping, and `regex_replace_*` - which now only materializes when a replacement actually occurred). Tokenize-heavy benchmarks: default path -20%, trailing-space -33% (now allocation free at plain-path speed), all-space -19%.
...
- Added `mbo::diff` output formats: next to the default unified format, `DiffOptions::output_format` selects context format (`diff -c`), normal format (plain `diff`), or side-by-side format (like `diff -y --expand-tabs`: two space-padded columns with a `|`/`<`/`>` gutter, total width via `DiffOptions::side_by_side_width` / `--width`, default 130; full files by default, an explicit `--context` renders only the chunks around changes; for eyeballing, not `patch`able). The `diff` binary and the `diff_test` bazel rule gained the matching `--format=unified|context|normal` flag / `format` attribute (normal format defaults `--context` to 0 and emits no file headers; context format uses `***`/`---` file headers). Both new formats reproduce GNU diff output byte for byte and apply cleanly with `patch`.
