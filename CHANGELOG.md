...
- Renamed the legacy in-house hash `mbo::hash::simple` to `mbo::hash::dumbo` (the -umbo family's weak member: 64-bit, unseeded, does not pass SMHasher3, kept for comparison only) and **removed** the deprecated `simple::GetHash` wrapper and the `mbo::hash::simple` namespace outright - pre-1.0, so no compatibility alias is kept and existing `mbo::hash::simple::*` users must migrate. Use `mbo::hash::GetHash64` (mumbo) for real hashing; `mbo::hash::dumbo::GetHash64` remains for comparison.
