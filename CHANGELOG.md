...
- Completed the `mbo/digest` algorithm set: **SHA-384/512/512-224/512-256** (FIPS 180-4, 64-bit words; the SHA-512/t IVs rederived programmatically per the spec), **SHA3-224/256/384/512** (FIPS 202 Keccak sponge, LFSR-generated round constants), **BLAKE2b** (RFC 7693; `blake2b` 64-byte and `blake2b_256` variants), **`Hmac<Algo>`/`HmacStreamer`** (RFC 2104, generic over any streaming digest incl. HMAC-SHA3 with rate-sized blocks), and **SHA-1** + **MD5** for legacy interop (both loudly marked collision-broken). All constexpr with compile-time == runtime proofs; every value pinned against independently generated vectors incl. per-algorithm padding boundaries and the million-'a' inputs. BLAKE3 (tree mode) remains planned.
- Added the `digest` binary (`//mbo/digest:digest`): checksum-style `<hash>  <file>` lines (`sha256sum`/`shasum` format, byte-compatible; `--reverse` swaps the columns), `-a`/`--algorithm` selects any of the 17 library algorithms (default sha256), `-` reads stdin, directories are errors; streaming chunked reads (no whole-file buffering).
- Added **BLAKE3** to `mbo/digest` (constexpr transcription of the spec; scalar/sequential with the full Merkle-tree structure): plain `Digest`/`DigestXof<N>`, native keyed mode (`DigestKeyed`/`StreamInitKeyed`), and the `DeriveKey<N>` KDF mode - all pinned against the official BLAKE3 test-vector suite (35 lengths x 3 modes, incl. 131-byte XOF outputs) with compile-time proofs per mode.
- Added **SHAKE128/SHAKE256** (FIPS 202 XOFs) to `mbo/digest`: `shake128::Digest<N>` / fixed-size `Algorithm<N>` on the shared Keccak sponge, with multi-block squeeze; outputs of different lengths share their prefix (documented).
- Added **native BLAKE2b keying** (RFC 7693 key block, keys up to 64 bytes): `DigestKeyed` / `StreamInitKeyed` on the BLAKE2b algorithms - BLAKE2 is its own MAC, no HMAC construction needed.
- Added the big-endian `hash_internal::Load64BE` (SHA-512-family word loads) beside `Load32BE`.
