...
- Restored the empty-input guards in log timing's reverse parsers, added direct regression tests,
  optimized `LimitedMap`/`LimitedSet` lower and upper bounds for all ordinary comparators on small
  containers, and covered their behavior without counting repeated capacity-specific template
  instantiations as distinct functions.
